...
Example: (define-column-names mtcars)"
  (maphash #'(lambda (key index)
	       (let ((col (intern (concatenate 'string (symbol-name df) "$" (symbol-name key)) package)))
		 (export col)
		 (eval `(cl:define-symbol-macro ,col (aref (columns ,df) ,index)))))
	   (ordered-keys-table (slot-value (symbol-value df) 'ordered-keys))))

(defun undef-column-names (df)
  "Remove symbol macro for each column name in DF"
  (maphash #'(lambda (key index)
	       (declare (ignore index))
	       (let ((sym (concatenate 'string (symbol-name df) "$" (symbol-name key))))
		 ;; (format t "Uninterning ~A " sym)
		 (unintern (find-symbol sym))))
	   (ordered-keys-table (slot-value (symbol-value df) 'ordered-keys))))

;;; TODO make this work on *variables* and *data-frames*
(defun undef (df)
